...
| Tutorial body | `_content/modules/tutorials/bbms1021/tN.qmd` (Tutorial 1 also `t1-1.qmd`) | include (no YAML); `{webr}` + `::: {.hide}` solutions |
| Student skin (published) | `courses/2026/BBMS1021/tutorial_N.qmd` | live-html + `tutorial_student.css` |
| RStudio worksheet / teacher notes | `_student/.../tutorial_N-worksheet.qmd`, `...-notes.qmd` | generated by `scripts/pack_bbms1021_tutorials.py`; `{r}`; not published |
...
For Tutorial 1 also include `t1-1.qmd`. RStudio `{r}` packs: `python scripts/pack_bbms1021_tutorials.py` (not a second website skin).
