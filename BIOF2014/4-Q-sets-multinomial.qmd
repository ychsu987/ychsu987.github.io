---
title: "Question sets-Multivariate Discrete Distributions"
...
The likelihood function for $N$ independent categorical observations is the product of the probabilities of the observed classes. Let $n_k$ be the count of observations in class $k$ such that $\sum_{k=1}^K n_k = N$.
$$ 
L(\mathbf{p} | X) = \prod_{i=1}^N P(x_i | \mathbf{p}) = \prod_{k=1}^K p_k^{n_k}
$$
...
$$
  \begin{aligned}
  P(\mathbf{p} | X) &\propto L(\mathbf{p} | X) \times P(\mathbf{p}) \\
...
2. Show that the posterior predictive probability of a new observation $\tilde{x}$ belonging to class $k$ is:
