...
::: {.hide}
Base R (works in WebR without extra packages):
...
![Illustration of LOOCV](/assets/images/bbms1021/loocv.png){width="568"}
...
::: {.hide}
```{webr}
data$predict[32] <- train_n_predict(data[-32, ], data[32, ])
data$predict[32]
```
:::
...
for (i in 1:nrow(data)) {
  data$predict[i] <- train_n_predict(data[-i, ], data[i, ])
}
```
:::
...
::: {.hide}
Suggested (vectorised):

```{webr}
data$p_class <- factor(data$predict >= 0.5, labels = c("automatic", "manual"))
```

A loop works but is harder to maintain (see Q6).
:::
...
::: {.hide}
```{webr}
accuracy <- sum(data$p_class == data$am) / nrow(data)
accuracy
```
:::
...
::: {.hide}
::: {.callout-note}
Harder to maintain, and usually slower than vectorised operations on whole columns.
:::
:::
