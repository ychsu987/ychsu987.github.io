...
::: {.callout-tip}
## Required takeaway
- **RR = 1**: similar risk in exposed and unexposed  
- **RR > 1 / RR < 1**: higher / lower risk in the exposed group  
- If a confidence interval for the effect **includes the null** (no difference), treat the result as **inconclusive**—not as proof of no effect.
:::

## (Optional deep dive — not assessed) Inference on RR

::: {.callout-note}
## Required takeaway vs optional math
**Required:** interpret RR (above/below 1) and that a confidence interval covering the null means the result is inconclusive.  
**Optional below:** SE formula, $z$-interval construction, and code. Not included in the exam (Z-test is an approximation; other methods adjust for covariates more cleanly).
:::
...
## (Optional deep dive — not assessed) Cohort and Poisson regression

::: {.callout-note}
## Optional modeling bridge
This section links RR to a Poisson regression coefficient $\beta_1$. Useful for later courses; **not** the core workshop skill. Core skill remains: build the $2\times 2$ table and interpret RR/OR.
:::
...
## Historical notes (optional context)
